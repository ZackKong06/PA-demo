    ...
    execute: function () {
      ...
      _export("ProceduralFactory", ProceduralFactory = class ProceduralFactory {
        ...
        createSoldier(name, parent, color) {
          ...
          var weapon = this.createBox('Blaster', root, new Vec3(0.32, 0.82, -0.24), new Vec3(0.12, 0.14, 0.62), new Color(38, 45, 54));
        ...
        createEnemy(name, parent, boss) {
          ...
          var root = new Node(name);
          root.setParent(parent);
          var bodyColor = boss ? new Color(123, 57, 190) : new Color(221, 69, 79);
          var darkColor = boss ? new Color(55, 31, 86) : new Color(82, 36, 41);
          var size = boss ? 1.75 : 1;
          ...
          if (boss) {
            var hornL = this.createCone('HornL', root, new Vec3(-0.28, 1.72, 0), new Vec3(0.18, 0.5, 0.18), new Color(250, 205, 93));
            var hornR = this.createCone('HornR', root, new Vec3(0.28, 1.72, 0), new Vec3(0.18, 0.5, 0.18), new Color(250, 205, 93));
        ...
        createGate(parent, value, color) {
          var root = new Node(value >= 0 ? "Gate+" + value : "Gate" + value);
          root.setParent(parent);
        ...
        updateGateValue(gate, value, color) {
          ...
          for (var partName of ['LeftPost', 'RightPost', 'Header']) {
            var _gate$getChildByName;

            var renderer = (_gate$getChildByName = gate.getChildByName(partName)) == null ? void 0 : _gate$getChildByName.getComponent(MeshRenderer);
            renderer == null || renderer.setMaterial(this.getMaterial(color), 0);
          }

          var sign = gate.getChildByName('Sign');

          if (!sign) {
            return;
          }

          for (var child of [...sign.children]) {
            child.destroy();
          }

          var digits = Math.floor(Math.abs(value)).toString();
          var digitsWidth = digits.length * 0.9 - 0.2;
          var groupWidth = 0.7 + 0.25 + digitsWidth;
          var left = -groupWidth * 0.5;
        ...
        createDigit(parent, value, x, color) {
          ...
          var digit = Math.max(0, Math.min(9, Math.floor(value)));
          var segmentPositions = [new Vec3(x + 0.35, 0.48, -0.18), new Vec3(x + 0.7, 0.24, -0.18), new Vec3(x + 0.7, -0.24, -0.18), new Vec3(x + 0.35, -0.48, -0.18), new Vec3(x, -0.24, -0.18), new Vec3(x, 0.24, -0.18), new Vec3(x + 0.35, 0, -0.18)];

          for (var index of segmentsByDigit[digit]) {
            var vertical = index === 1 || index === 2 || index === 4 || index === 5;
