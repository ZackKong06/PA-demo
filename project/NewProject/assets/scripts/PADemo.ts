...
export class PADemo extends Component {
    ...
    private createBuilding(side: number, z: number, index: number): Node {
    ...
    private createTree(side: number, z: number, index: number): Node {
    ...
    private createMountain(side: number, z: number, index: number): Node {
    ...
    private randomLane(seed: number): number {
        return this.seededRandom(seed) * 4.4 - 2.2;
    }

    private spawnWall(x: number, z: number): void {
    ...
    private spawnWoodFence(x: number, z: number): void {
        ...
        for (let row = 0; row < 3; row++) {
            this.factory.createBox(
                'Fence Piece',
                node,
                new Vec3(0, 0.36 + row * 0.43, 0),
                new Vec3(2.35, 0.18, 0.2),
                row % 2 === 0 ? lightWood : wood,
            );
        }
    ...
    private spawnAgilityPickup(x: number, z: number): void {
        ...
        this.agilityPickups.push({ node, width: 0.55, collected: false });
    }

    private spawnWave(z: number, count: number, speed: number): void {
    ...
    private spawnGiant(z: number, x: number): void {
    ...
    private spawnBrute(z: number, x: number): void {
            ...
            if (!enemy.crossedPlayer && z >= this.playerZ - 0.45) {
                ...
                if (touchesSquad) {
                    ...
                    const launchTarget = new Vec3(
                        x + directionX * knockDistance,
                        knockHeight,
                        z + directionZ * knockDistance,
                    );
                    const vanishTarget = new Vec3(
                        launchTarget.x + directionX * 1.8,
                        launchTarget.y + 1.1,
                        launchTarget.z + directionZ * 1.8,
                    );
    ...
    private updateSpeedAura(): void {
        this.speedAura?.destroy();
    ...
    private spawnDustCloud(position: Readonly<Vec3>): void {
        const dustColors = [
            new Color(132, 91, 55),
            new Color(163, 116, 70),
            new Color(104, 72, 48),
        ];
        for (let index = 0; index < 12; index++) {
            const angle = (Math.PI * 2 * index) / 12;
            const distance = 1.1 + (index % 4) * 0.34;
            const dust = this.factory.createSphere(
                'Dust',
                this.effectRoot,
                new Vec3(position.x, 0.25 + (index % 3) * 0.18, position.z),
                new Vec3(0.22, 0.16, 0.22),
            ...
            tween(dust)
                ...
                .to(0.28, { scale: new Vec3(0.02, 0.02, 0.02) }, { easing: 'quadIn' })
                .call(() => dust.destroy())
                .start();
        }
    }

    private spawnBloodParticles(position: Readonly<Vec3>, strength: number): void {
        ...
        for (let index = 0; index < count; index++) {
            ...
            const particle = this.factory.createSphere(
                'Red Impact Particle',
                this.effectRoot,
                new Vec3(position.x, 0.85, position.z),
                new Vec3(0.09, 0.09, 0.09),
                index % 2 === 0 ? new Color(218, 38, 48) : new Color(154, 18, 32),
            );
            tween(particle)
                .to(
                    0.38 + (index % 3) * 0.08,
        ...
        for (let index = 0; index < pieces.length; index++) {
            ...
            tween(piece)
                .to(
                    ...
                    {
                        position: new Vec3(
                            start.x + Math.cos(angle) * (2 + (index % 2) * 0.7),
                            1.5 + (index % 3) * 0.65,
                            start.z + Math.sin(angle) * (2 + (index % 2) * 0.7),
                        ),
    ...
    private shatterWall(wall: Node): void {
        ...
        for (let index = 0; index < blocks.length; index++) {
            ...
            const vanishTarget = new Vec3(
                launchTarget.x + Math.cos(angle) * 1.2,
                launchTarget.y + 0.8,
                launchTarget.z + Math.sin(angle) * 1.2,
            );
            block.setRotationFromEuler(index * 37, index * 53, index * 23);
            tween(block)
                .to(
                    0.7 + (index % 3) * 0.1,
                    { position: launchTarget, scale: new Vec3(0.28, 0.28, 0.28) },
                    { easing: 'quadOut' },
                )
                .to(
                    0.42 + (index % 2) * 0.1,
                    { position: vanishTarget, scale: new Vec3(0.03, 0.03, 0.03) },
                    { easing: 'quadIn' },
                )
                .call(() => block.destroy())
                .start();
